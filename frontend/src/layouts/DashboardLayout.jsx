  ...
  return (
    <div className={`flex min-h-screen font-sans transition-colors duration-300 ${darkMode ? 'bg-slate-900' : 'bg-[#F8FAFF]'}`}>
      ...
      <div className="flex-1 ml-64 flex flex-col">
        {/* Top Navbar */}
        <header className={`h-20 backdrop-blur-md border-b px-10 flex items-center justify-between sticky top-0 z-40 transition-colors duration-300 ${darkMode ? 'bg-slate-800/80 border-slate-700' : 'bg-white/50 border-slate-100'}`}>
          ...
          <div className={`flex items-center gap-6 ${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>
            <div className="relative">
              ...
              {showNotifications && notifications.length > 0 && (
                <div className={`absolute right-0 top-full mt-2 w-80 border rounded-3xl shadow-xl p-4 z-50 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
        ...
        <main className="p-10">
          <div className="mb-8">
            <h1 className={`text-3xl font-black tracking-tight ${darkMode ? 'text-white' : 'text-slate-900'}`}>{title}</h1>
          </div>
          {children}
        </main>
      </div>
